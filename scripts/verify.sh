...
docker compose exec -T database mariadb -N \
    ...
    -e "SELECT COUNT(*) FROM openvillage_schema_migrations WHERE version IN ('001_ajax_token_length.sql', '002_adventure_uid_unsigned.sql', '003_scheduled_task_failures.sql', '004_brewery_festivals.sql', '005_npc_players.sql', '006_village_check_and_artifact_flag.sql');" \
    | rg -q '^6$'
